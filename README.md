...
Closing Agentaps ends mobile access until the app runs again. To rotate credentials and revoke pairing links, close Agentaps and remove the `MOBILE_CREDENTIALS` entry for project `agentaps` and profile `default` from your configured provider before starting it again.
