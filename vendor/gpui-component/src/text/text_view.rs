        ...
        loop {
            match self.rx.poll_next(cx) {
                Poll::Ready(Some(update)) => {
                    ...
                    // Keep the first timer so a steady stream cannot postpone parsing forever.
                    // See https://github.com/longbridge/gpui-kit/issues/2634.
                    if changed && !self.parse_scheduled {
...
pub(crate) struct TextViewState {
    ...
    last_submitted_text: Option<SharedString>,
...
impl TextView {
    ...
    pub fn markdown(
        ...
        if state.read(cx).last_submitted_text.as_ref() != Some(&markdown) {
            state.update(cx, |state, _| {
                state.last_submitted_text = Some(markdown.clone());
                if let Some(tx) = &state.tx {
