...
impl WebWindowInner {
    ...
    fn register_pointer_down(self: &Rc<Self>) -> EventListenerHandle {
        ...
        self.listen("pointerdown", move |event: JsValue| {
            ...
            if is_touch(&event) {
                if this.touch_pointer.get().is_none() {
                    let position = pointer_position_in_element(&event);
                    this.touch_pointer.set(Some(TouchPointer {
                        id: event.pointer_id(),
                        start: position,
                        last: position,
                        moved: false,
    ...
    fn register_pointer_up(self: &Rc<Self>) -> EventListenerHandle {
        ...
        self.listen("pointerup", move |event: JsValue| {
            ...
            if is_touch(&event) {
                let Some(touch) = this.touch_pointer.get() else {
                    return;
                };
                if touch.id != event.pointer_id() {
                    return;
                }
                this.touch_pointer.set(None);
                let position = pointer_position_in_element(&event);
                if touch.moved {
                    ...
                    let bounds_height = this.state.borrow().bounds.size.height;
                    let near_composer = position.y >= bounds_height - px(140.);
                    if near_composer || this.state.borrow().input_handler.is_some() {
                        this.input_element.focus().ok();
                    }
                }
                return;
    ...
    fn register_pointer_cancel(self: &Rc<Self>) -> EventListenerHandle {
        ...
        self.listen("pointercancel", move |event: JsValue| {
            let event: web_sys::PointerEvent = event.unchecked_into();
            if is_touch(&event) {
                if let Some(touch) = this.touch_pointer.get()
                    && touch.id == event.pointer_id()
                {
                    this.touch_pointer.set(None);
                    if touch.moved {
                        this.dispatch_input(PlatformInput::ScrollWheel(ScrollWheelEvent {
    ...
    fn register_pointer_move(self: &Rc<Self>) -> EventListenerHandle {
        ...
        self.listen("pointermove", move |event: JsValue| {
            ...
            if is_touch(&event) {
                let Some(mut touch) = this.touch_pointer.get() else {
                    return;
                };
                if touch.id != event.pointer_id() {
                    return;
                ...
                let position = pointer_position_in_element(&event);
                if !touch.moved {
                    let dx = f32::from(position.x - touch.start.x);
                    let dy = f32::from(position.y - touch.start.y);
                    if dx * dx + dy * dy < 64. {
                        return;
                    }
                }
                let phase = if touch.moved {
                    TouchPhase::Moved
                } else {
                    TouchPhase::Started
                };
                let delta = position - touch.last;
                touch.last = position;
                touch.moved = true;
                this.touch_pointer.set(Some(touch));
                this.dispatch_input(PlatformInput::ScrollWheel(ScrollWheelEvent {
                    position,
    ...
    /// Phone keyboards often emit `beforeinput` without a useful `keydown`.
    /// Route those edits through the same GPUI input handler as desktop keys.
