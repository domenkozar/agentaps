...
static BUNDLED_FONTS: &[&[u8]] = &[include_bytes!("../../../web/fonts/IBMPlexSans-Regular.ttf")];
...
impl Platform for WebPlatform {
    ...
    fn run(&self, on_finish_launching: Box<dyn 'static + FnOnce()>) {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            let graphics =
                match prepare_graphics(&browser_window, wgpu::Backends::BROWSER_WEBGPU).await {
                    Ok(graphics) => Ok(graphics),
                    Err(webgpu_error) => {
                        log::warn!("WebGPU initialization failed; trying WebGL2: {webgpu_error:#}");
                        prepare_graphics(&browser_window, wgpu::Backends::GL)
                        .await
                        .map_err(|webgl_error| {
                            anyhow::anyhow!(
            ...
            match graphics {
                Ok((context, window)) => {
                    log::info!(
                        "Browser graphics initialized with {:?}",
                        context.adapter.get_info().backend
                    );
                    *wgpu_context.borrow_mut() = Some(context);
                    *prepared_window.borrow_mut() = Some(window);
                    on_finish_launching();
                }
                Err(err) => {
                    log::error!("Failed to initialize browser graphics: {err:#}");
    ...
    fn open_window(
        ...
        let context = context_ref.as_ref().ok_or_else(|| {
            anyhow::anyhow!("Browser graphics context not initialized. Was Platform::run() called?")
        })?;
        ...
        let prepared = self
            ...
            .borrow_mut()
            .take()
            .ok_or_else(|| anyhow::anyhow!("Only one browser window is supported"))?;
...
async fn prepare_graphics(
    browser_window: &web_sys::Window,
    backend: wgpu::Backends,
) -> anyhow::Result<(WgpuContext, PreparedWebWindow)> {
    let canvas = WebWindow::prepare_canvas(browser_window)?;
    match WgpuContext::new_web_with_surface(&canvas, backend).await {
        Ok((context, surface)) => Ok((context, PreparedWebWindow { canvas, surface })),
        Err(error) => {
            canvas.remove();
            Err(error)
        }
...
fn show_graphics_unavailable_message(browser_window: &web_sys::Window) {
    ...
    message.set_text_content(Some(
        "Failed to initialize browser graphics. This application requires WebGPU or WebGL2.",
    ));
