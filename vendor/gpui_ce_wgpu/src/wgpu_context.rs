...
impl WgpuContext {
    ...
    /// Create a browser adapter for one graphics backend and the canvas it will present to.
    /// WebGL2 requires a compatible surface during adapter selection.
    #[cfg(target_family = "wasm")]
    pub async fn new_web_with_surface(
        ...
        let surface = instance
            ...
            .map_err(|error| anyhow::anyhow!("Failed to create browser surface: {error}"))?;
        let adapter = instance
            .request_adapter(&wgpu::RequestAdapterOptions {
                power_preference: wgpu::PowerPreference::HighPerformance,
            ...
            .await
            .map_err(|error| anyhow::anyhow!("Failed to request browser adapter: {error}"))?;
    ...
    async fn create_device(
        ...
        #[cfg(target_family = "wasm")]
        let baseline_limits = if adapter.get_info().backend == wgpu::Backend::Gl {
            wgpu::Limits::downlevel_webgl2_defaults()
        } else {
            wgpu::Limits::downlevel_defaults()
        };
        #[cfg(not(target_family = "wasm"))]
        let baseline_limits = wgpu::Limits::downlevel_defaults();

        let mut required_limits = baseline_limits
