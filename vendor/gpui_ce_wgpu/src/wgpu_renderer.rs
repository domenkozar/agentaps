...
const MAX_INSTANCE_DATA_SIZE: u64 = 256 * 1024 * 1024;
...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        let uses_webgl_instance_data = context.adapter.get_info().backend == wgpu::Backend::Gl;
        ...
        let (
            ...
            let max_size = (u64::from(max_dimension).pow(2) * INSTANCE_TEXTURE_TEXEL_SIZE)
                .min(MAX_INSTANCE_DATA_SIZE);
            let (data, capacity) = Self::create_instance_texture(
                &device,
                (2 * 1024 * 1024).min(max_size),
                max_dimension,
            );
            (data, capacity, max_size, INSTANCE_TEXTURE_TEXEL_SIZE)
        } else {
            let max_size = device
                .limits()
                .max_buffer_size
                ...
                .min(MAX_INSTANCE_DATA_SIZE);
            let capacity = (2 * 1024 * 1024).min(max_size);
    ...
    fn create_instance_texture(
        ...
        let texels = requested_capacity.div_ceil(INSTANCE_TEXTURE_TEXEL_SIZE);
        let width = texels.min(u64::from(max_texture_dimension)).max(1) as u32;
        let height = texels
    ...
    fn create_bind_group_layouts(
        ...
        let instances = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[instance_data_entry.clone()],
        });
    ...
    fn create_pipelines(
        ...
        let instance_shader_source = if uses_webgl_instance_data {
    ...
    fn draw_instances(
        ...
        let stride = data.len() as u64 / u64::from(instance_count);
        let Some((offset, size, first_instance)) =
            self.write_to_instance_buffer(instance_offset, data, stride)
        else {
            return false;
        };
        let resources = self.resources();
    ...
    fn draw_paths_to_intermediate(
        ...
        let Some((vertex_offset, vertex_size, first_vertex)) = self.write_to_instance_buffer(
            instance_offset,
            vertex_data,
    ...
    fn grow_instance_buffer(&mut self) {
        ...
        let resources = self.resources_mut();
        if uses_webgl_instance_data {
            let max_dimension = resources.device.limits().max_texture_dimension_2d;
            let (data, capacity) =
                Self::create_instance_texture(&resources.device, new_capacity, max_dimension);
            resources.instance_data = data;
    ...
    fn write_to_instance_buffer(
        ...
        data: &[u8],
        stride: u64,
    ) -> Option<(u64, NonZeroU64, u32)> {
        let alignment = if self.uses_webgl_instance_data {
            least_common_multiple(self.storage_buffer_alignment, stride.max(1))
        } else {
            self.storage_buffer_alignment.max(1)
        };
        ...
        if offset + allocation_size > self.instance_buffer_capacity {
            return None;
        }
        let resources = self.resources();
        match &resources.instance_data {
            InstanceData::Storage(buffer) => resources.queue.write_buffer(buffer, offset, data),
            InstanceData::Texture { .. } => Self::write_instance_texture(resources, offset, data),
        }
        *instance_offset = offset + allocation_size;
        let first_instance = if self.uses_webgl_instance_data {
            u32::try_from(offset / stride).ok()?
        } else {
            0
        };
        Some((
            offset,
            NonZeroU64::new(size).expect("size is at least 16"),
            first_instance,
        ))
    }
